...
- Generate floorplan-based images and selected-area edits with the Image API edits endpoint (default model: `gpt-image-1.5`, with server fallback to `gpt-image-1` if OpenAI rejects the model).
...
If you connect the repo through the Vercel dashboard, set `OPENAI_API_KEY` and `OPENAI_IMAGE_MODEL=gpt-image-1.5` in Project Settings -> Environment Variables. If you later enable OpenRouter planning, also set `OPENROUTER_API_KEY` there. Vercel will serve `public/index.html` and the API routes in `api/`.
...
- `gpt-image-1.5` is used by default through `OPENAI_IMAGE_MODEL`. If OpenAI rejects that model for a request, the server retries once with `gpt-image-1`.
- `gpt-image-2` and `gpt-image-2.0` are accepted as aliases and mapped to `gpt-image-1.5`.
...
- `GET /api/health`
  - Returns: `{ ok: true, imageModel }` for quick model verification in Vercel/local
